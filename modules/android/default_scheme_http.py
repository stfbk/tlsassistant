...
class DefaultSchemeHttp(Sebastian_base):
    """
    Check if the app uses HttpHost but its default scheme is HTTP.

    """

    stix = Bundled(mitigation_object=load_mitigation("DEFAULT_SCHEME_HTTP"))

    def _get_logger(self):
        """
        Returns the module logger
        """
        return Logger("DefaultSchemeHttp")
    ...
    def _set_mitigations(self, result: dict, key: str, condition: bool) -> dict:
        ...
        if condition:
            result["mitigation"] = load_mitigation(
                "DEFAULT_SCHEME_HTTP", raise_error=False
            ) 
