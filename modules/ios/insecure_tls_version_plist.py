...
class InsecureTlsVersionPlist(Sebastian_base):
    """
    Checks if the app set minimum value of TLS version TLSv1.0 or TLSv1.1, which are unsafe.
    """

    stix = Bundled(mitigation_object=load_mitigation("INSECURE_TLS_VERSION_PLIST"))

    def _get_logger(self):
        """
        Returns the module logger
        """
        return Logger("InsecureTlsVersionPlist")
    ...
    def _set_mitigations(self, result: dict, key: str, condition: bool) -> dict:
        ...
        if condition:
            result["mitigation"] = load_mitigation(
                "INSECURE_TLS_VERSION_PLIST", raise_error=False
            ) 
    ...
    def _worker(self, results):
        ...
        return self._obtain_results(
            results, ["InsecureTlsVersionPlist"]
        )
