...
for host in hosts:
    sighting,group,object_refs,observed_data = Stix().run(modules=modules,hostname_or_path=host)
...
for module,loaded_module in modules.items():
    sighting,object_refs,observed_data = Stix().run(type_of_analysis=Stix.Type.MODULES,module=module,loaded_module=loaded_module, hostnames_or_paths = hosts)
